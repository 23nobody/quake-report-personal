    ...
    public static ArrayList<Word> extractEarthquakes() {
        ...
        try {
            ...
            for(int i=0; i < jsonArray.length(); i++) {
                ...
                JSONObject prop = jsonObject.optJSONObject("properties");
                double mag = Double.parseDouble(prop.optString("mag"));
                String place = prop.optString("place").toString();
                long time = Long.parseLong(prop.optString("time").toString());
                String url = prop.optString("url");
                earthquakes.add(new Word(mag,place,time,url));
